...
from scratch.batch_3 import BATCH_3_PEDS, BATCH_3_OBGYN, BATCH_3_MSK_DERM

# To prevent duplicates if run multiple times, check syndrome_id
existing_ids = {s['syndrome_id'] for s in SYNDROME_KB}

for s in BATCH_3_PEDS + BATCH_3_OBGYN + BATCH_3_MSK_DERM:
    if s['syndrome_id'] not in existing_ids:
        # Format differentials before adding
        new_diffs = []
